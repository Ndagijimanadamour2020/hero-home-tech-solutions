...
export default async function AdminPage() {
  ...
  const [leads, newLeads, solutions, events, recentLeads] = await Promise.all([prisma.lead.count(), prisma.lead.count({ where: { status: 'NEW' } }), prisma.solution.count(), prisma.analyticsEvent.count({ where: { createdAt: { gte: new Date(Date.now() - 30 * 86400000) } } }), prisma.lead.findMany({ orderBy: { createdAt: 'desc' }, take: 8 })]);
  const metrics = [['Total leads', leads], ['New leads', newLeads], ['Published solutions', solutions], ['Events, last 30 days', events]];
